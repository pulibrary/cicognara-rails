...
RSpec.describe Entry, type: :model do
  ...
  let(:contrib) { ContributingLibrary.find_or_create_by! label: 'Example Library', uri: 'http://example.org' }
  let(:book) { Book.first }
  let(:version) do
  ...
  describe '#to_solr' do
    ...
    context 'when @corresp links to a MARC record' do
      ...
      it 'indexes 024$a values using the dclib_display field' do
        expect(solr_document['alt_id']).to eq(['cico:m87'])
      end

      it 'indexes the MARC record XPath using the file_uri_s field' do
        expect(solr_document['file_uri_s']).to eq("file://#{marc}//marc:record[0]")
      end
    end
    ...
    context 'when the entry has a matching version copy' do
      subject(:entry) { c1d1e444 }
