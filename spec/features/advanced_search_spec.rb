...
RSpec.describe 'searching', type: :feature do
  ...
  it 'uses boolean operators to search across fields' do
    ...
    expect(page).to have_current_path('/catalog?utf8=%E2%9C%93&q=&search_field=advanced&f1=author&q1=GHIBERTI&op2=NOT&f2=author&q2=Lorenzo&op3=AND&f3=title&q3=&range[pub_date][begin]=&range[pub_date][end]=&sort=score+desc%2C+title_sort+asc&commit=Search')
  end
  ...
  it 'permits users to filter by facets' do
    visit '/catalog?utf8=%E2%9C%93&q=&search_field=all_fields&f1=all_fields&q1=&op2=AND&f2=author&q2=&op3=AND&f3=title&q3=&f_inclusive%5Bname_facet%5D%5B%5D=Agrippa+von+Nettesheim%2C+Heinrich+Cornelius%2C+1486%3F-1535&range%5Bpub_date%5D%5Bbegin%5D=&range%5Bpub_date%5D%5Bend%5D=&sort=score+desc%2C+title_sort+asc&search_field=advanced&commit=Search'
  ...
  it 'combines multiple queries performed on the same field' do
    ...
    find('#advanced-search-submit').click
    expect(page).to have_current_path('/catalog?utf8=%E2%9C%93&q=&search_field=advanced&f1=author&q1=Agrippae&op2=AND&f2=author&q2=Henrici&op3=AND&f3=author&q3=Corn&range[pub_date][begin]=&range[pub_date][end]=&sort=score+desc%2C+title_sort+asc&commit=Search')
  end
  ...
  context 'when searching with three queries' do
    ...
    it 'combines multiple queries with the default AND operator' do
      ...
      find('#advanced-search-submit').click
      expect(page).to have_current_path('/catalog?utf8=%E2%9C%93&q=&search_field=advanced&f1=all_fields&q1=Agrippae&op2=AND&f2=author&q2=Henrici&op3=AND&f3=title&q3=Corn&range[pub_date][begin]=&range[pub_date][end]=&sort=score+desc%2C+title_sort+asc&commit=Search')
    end
  end
  ...
  context 'when searching with three queries using the NOT operator' do
    ...
    it 'combines multiple queries with the NOT operator' do
      ...
      find('#advanced-search-submit').click
      expect(page).to have_current_path('/catalog?utf8=%E2%9C%93&q=&search_field=advanced&f1=all_fields&q1=Agrippae&op2=NOT&f2=author&q2=Henrici&op3=NOT&f3=title&q3=Corn&range[pub_date][begin]=&range[pub_date][end]=&sort=score+desc%2C+title_sort+asc&commit=Search')
    end
  end
end
